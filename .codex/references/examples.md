...
Custom firmware supplied with `--firmware HEX` is not generated by `npm run setup`. Use the project's generation procedure; for magic-circle, run `npm run firmware:magic-circle`.
